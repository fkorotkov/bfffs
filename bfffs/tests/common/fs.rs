...
"#;
    ...
    test rename_dir_to_nonemptydir(mocks) {
        ...
        assert_eq!(mocks.val.0.rename(&srcdir_fd, &src_fd, &src,
            &dstdir_fd, Some(dst_fd), &dst),
            Err(libc::ENOTEMPTY)
        );
    ...
    test rename_dot(mocks) {
        ...
        assert_eq!(Err(libc::EINVAL),
            mocks.val.0.rename(&srcdir_fd, &srcdir_fd, &dot, &srcdir_fd,
                None, &dst)
        );
    }
    ...
    test rename_dotdot(mocks) {
        ...
        assert_eq!(Err(libc::EINVAL),
            mocks.val.0.rename(&srcdir_fd, &root, &dotdot, &srcdir_fd,
                None, &dst)
        );
    }
    ...
    test rename_nondir_to_hardlink(mocks) {
        ...
        assert_eq!(src_fd.ino(),
            mocks.val.0.rename(&root, &src_fd, &src, &root, Some(dst_fd),
                &dst).unwrap()
        );
