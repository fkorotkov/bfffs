...
impl Fs {
    ...
    pub fn rename(&self, parent: &FileData, fd: &FileData, name: &OsStr,
        newparent: &FileData, newfd: Option<FileData>, newname: &OsStr)
        -> Result<u64, i32>
    {
        // Outline:
        // 0)  Check conditions
        ...
        let ino = fd.ino();
        let dst_ino = newfd.map(|fd| fd.ino);
        let parent_ino = parent.ino;
...
fn rename_eio() {
    ...
    let fs = Fs::new(Arc::new(db), rt.handle().clone(), tree_id);
    let root = fs.root();
    let dst_fd = FileData::new_for_tests(Some(root.ino), dst_ino);
    let fd = FileData::new_for_tests(Some(root.ino), src_ino);
    let r = fs.rename(&root, &fd, &srcname, &root, Some(dst_fd), &dstname);
